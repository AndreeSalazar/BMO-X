...
impl JavaToNexo {
    ...
    fn translate_class(&self, cls: &JClass) -> BxResult<crate::lang::bmo::parser::Stmt> {
        // Lower to: a struct with embedded parent + vptr + data fields.
        let mut nexo_fields = Vec::new();
        if let Some(parent) = &cls.parent {
            nexo_fields.push(("base".to_string(), crate::lang::bmo::parser::TypeAnnotation::Named(parent.clone())));
        }
        let mut vt_name = cls.name.clone();
        vt_name.push_str("_vtable");
        nexo_fields.push(("vptr".to_string(), crate::lang::bmo::parser::TypeAnnotation::Named(vt_name)));
        for m in &cls.members {
            if let JMemberKind::Field { ty, name, .. } = &m.kind {
                nexo_fields.push((name.clone(), java_type_to_typeannotation(ty)));
            }
        }
        Ok(crate::lang::bmo::parser::Stmt::StructDecl {
            name: cls.name.clone(),
            fields: nexo_fields,
        })
    }
...
fn java_type_to_typeannotation(ty: &JType) -> crate::lang::bmo::parser::TypeAnnotation {
    use crate::lang::bmo::parser::TypeAnnotation;
    use JType::*;
    match ty {
