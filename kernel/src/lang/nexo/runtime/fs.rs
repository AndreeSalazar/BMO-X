...
pub fn open(name: &str) -> Result<FileHandle> {
    ...
    let fd = crate::fs::ramdisk::open(name_ptr, name_len);
    if fd == u64::MAX {
        Err(Error::NotFound)
    } else {
        Ok(FileHandle(fd))
...
pub fn read(fd: FileHandle, buf: &mut [u8]) -> Result<usize> {
    ...
    let n = crate::fs::ramdisk::read(fd.0, ptr, len);
    if n == u64::MAX {
        Err(Error::IoError)
    } else {
        Ok(n as usize)
...
pub fn close(fd: FileHandle) -> Result<()> {
    let ret = crate::fs::ramdisk::close(fd.0);
    if ret == u64::MAX {
        Err(Error::BadHandle)
    } else {
        Ok(())
...
pub fn size(fd: FileHandle) -> Result<u64> {
    let s = crate::fs::ramdisk::size(fd.0);
    if s == u64::MAX {
        Err(Error::BadHandle)
    } else {
        Ok(s)
...
pub fn init() {
    crate::diag::info("nexo_fs", "Filesystem subsystem initialized");
}
