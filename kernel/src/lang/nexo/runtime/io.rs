...
impl Graphics {
    ...
    pub fn new() -> Option<Self> {
        ...
        let fb = crate::ui::fb::Framebuffer::new(addr, stride as u64, width, height);
        Some(Self { fb })
    }
    ...
    pub fn draw_text(&self, x: usize, y: usize, text: &str, color: u32, scale: usize) {
        ...
        for byte in text.bytes() {
            let glyph = crate::ui::font::get_glyph(byte);
            for gy in 0..16 {
                let row = glyph[gy];
                for gx in 0..8 {
                    if row & (0x80 >> gx) != 0 {
...
pub fn beep(freq: u32, duration_ms: u32) {
    crate::desktop::beep(freq, duration_ms);
}

/// Initialize I/O subsystem.
pub fn init() {
    crate::diag::info("nexo_io", "I/O subsystem initialized");
}
