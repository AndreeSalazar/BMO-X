...
fn ser_hex(val: u64) {
    ...
    for i in (0..16).rev() {
        crate::dev::console::serial_write_byte(hex[((val >> (i * 4)) & 0xF) as usize]);
    }
}
...
pub unsafe fn init(
    ...
    crate::dev::console::serial_write("[page_alloc] Memory map entries: ");
    ser_usize(count);
    ...
    for entry in entries {
        match entry.mem_type {
            MemoryType::Usable => {
                ...
                if usable_count <= 10 {
                    crate::dev::console::serial_write("  [Usable] base=");
                    ser_hex(entry.base);
    ...
    crate::dev::console::serial_write("[page_alloc] Usable=");
    ser_u64(usable_count);
    crate::dev::console::serial_write(" (");
    ser_u64(usable_total_bytes / (1024 * 1024));
    crate::dev::console::serial_write(" MB) Reserved=");
    ser_u64(reserved_count);
    crate::dev::console::serial_write(" Other=");
    ser_u64(other_count);
    crate::dev::console::serial_write("\n");
    ...
    crate::dev::console::serial_write("[page_alloc] Init complete: free_pages=");
