...
pub fn init(features: &CpuFeatures) -> bool {
    if !features.has_avx || !features.has_osxsave {
        crate::device::serial::serial_write("[cpu] XCR0: skipped (no AVX/OSXSAVE)\n");
        return false;
    }
    ...
    unsafe {
        ...
        if cr4 & (1 << 18) == 0 {
            crate::device::serial::serial_write("[cpu] XCR0: FAIL — CR4.OSXSAVE not set!\n");
            return false;
        }
    ...
    crate::device::serial::serial_write("[cpu] XCR0 configured (x87 + SSE + AVX)\n");
    true
}
