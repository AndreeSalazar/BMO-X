...
//!
//! Design:
//!   - Tracks physical pages from 16 MB (`base`) to 4 GB (`MAX_ADDR`).
...
/// Convert a physical address to a page index relative to `base`.
...
fn ser_hex(val: u64) {
    ...
    for i in (0..16).rev() {
        crate::device::serial::serial_write_byte(hex[((val >> (i * 4)) & 0xF) as usize]);
    }
}
...
pub unsafe fn init(
    ...
    crate::device::serial::serial_write("[page_alloc] Memory map entries: ");
    ser_usize(count);
    ...
    for entry in entries {
        match entry.mem_type {
            MemoryType::Usable => {
                ...
                if usable_count <= 10 {
                    crate::device::serial::serial_write("  [Usable] base=");
                    ser_hex(entry.base);
                    crate::device::serial::serial_write(" size=");
                    ser_u64(entry.size / 1024);
                    crate::device::serial::serial_write(" KB\n");
                }
            }
    ...
    crate::device::serial::serial_write("[page_alloc] Usable=");
    ser_u64(usable_count);
    crate::device::serial::serial_write(" (");
    ser_u64(usable_total_bytes / (1024 * 1024));
    crate::device::serial::serial_write(" MB) Reserved=");
    ser_u64(reserved_count);
    crate::device::serial::serial_write(" Other=");
    ser_u64(other_count);
    crate::device::serial::serial_write("\n");
    ...
    crate::device::serial::serial_write("[page_alloc] Init complete: free_pages=");
    ser_usize(FREE_PAGES_COUNT);
    crate::device::serial::serial_write(" (");
    let mb = (FREE_PAGES_COUNT * 4096) / (1024 * 1024);
    ser_usize(mb);
    crate::device::serial::serial_write(" MB)\n");
}
