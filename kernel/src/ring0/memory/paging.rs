...
pub unsafe fn create_kernel_page_table() -> Option<u64> {
    ...
    crate::device::serial::serial_write("[paging] create_kernel_page_table: STUBBED (use UEFI PML4)\n");
    None
}
...
fn print_hex_debug(val: u64) {
    ...
    crate::device::serial::serial_write(unsafe {
...
unsafe fn alloc_page_table() -> Option<u64> {
    ...
    let raw = crate::memory::heap::heap_alloc(8192, 8);
    crate::device::serial::serial_write("[alloc_pt] raw=");
    crate::device::serial::serial_write("0x");
    print_alloc_hex(raw as u64);
    crate::device::serial::serial_write(" null=");
    crate::device::serial::serial_write(if raw.is_null() { "yes" } else { "no" });
    ...
    crate::device::serial::serial_write("[alloc_pt] aligned=0x");
    print_alloc_hex(aligned_addr as u64);
    crate::device::serial::serial_write(" pad=");
    crate::device::serial::serial_write(if aligned_addr - raw_addr >= 8 { "ok" } else { "no" });
    crate::device::serial::serial_write("\n");
    ...
    crate::device::serial::serial_write("[alloc_pt] about to write_bytes(aligned, 0, 4096)\n");
    core::ptr::write_bytes(aligned, 0, 4096);
    crate::device::serial::serial_write("[alloc_pt] write_bytes OK\n");
    Some(aligned as u64)
}
    ...
    for pml4_i in 0..256 {
        ...
        for pdpt_i in 0..512 {
            ...
            for pd_i in 0..512 {
                ...
                // 2 MiB huge page
                if (pde.0 & flags::HUGE_PAGE) != 0 {
                    crate::memory::page_alloc::free_pages(pde.phys_addr(), 512);
                    pde.0 = 0;
                    continue;
                }
                ...
                for pt_i in 0..512 {
                    let pte = &mut (*pt).entries[pt_i];
                    if pte.is_present() {
                        crate::memory::page_alloc::free_pages(pte.phys_addr(), 1);
                        pte.0 = 0;
                    }
                }
                crate::memory::page_alloc::free_pages(pde.phys_addr(), 1);
                pde.0 = 0;
            }
            crate::memory::page_alloc::free_pages(pdpte.phys_addr(), 1);
            pdpte.0 = 0;
        }
        crate::memory::page_alloc::free_pages(pml4e.phys_addr(), 1);
        pml4e.0 = 0;
    }
}
...
pub unsafe fn resolve_demand_page(
    ...
    let phys = crate::memory::page_alloc::alloc_pages_contiguous(1)
...
pub unsafe fn resolve_cow_page(
    ...
    let new_phys = crate::memory::page_alloc::alloc_pages_contiguous(1)
