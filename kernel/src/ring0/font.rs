...
//!
//! Covers printable ASCII 32 (space) through 126 (~). Based on the
//! standard IBM VGA 8x16 font (public domain).

/// Returns the 16-byte glyph bitmap for an ASCII character.
/// Printable range: 32..=127. Anything else returns blank.
pub fn get_glyph(ch: u8) -> &'static [u8; 16] {
    if ch < 32 || ch > 126 {
        return &GLYPHS[0]; // space
    }
    &GLYPHS[(ch - 32) as usize]
}
