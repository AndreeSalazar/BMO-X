...
pub extern "C" fn ap_main(lapic_id: u32) -> ! {
    ...
    serial::serial_write("[smp] AP alive, LAPIC ID=");
...
pub unsafe fn smp_init() {
    serial::serial_write("[smp] Starting AP bring-up (INIT-SIPI-SIPI)...\n");

    let max_cores = detect_core_count();
    let cores_to_start = core::cmp::min(max_cores - 1, MAX_AP);
    if cores_to_start == 0 {
        serial::serial_write("[smp] Only 1 core detected; no APs to start.\n");
        return;
    }

    serial::serial_write("[smp] Detected ");
    serial_write_num(cores_to_start as u64);
    serial::serial_write(" AP(s) to start.\n");

    setup_ap_gdt();
    setup_trampoline();

    for i in 0..cores_to_start {
        let stack = page_alloc::alloc_pages_contiguous(AP_STACK_SIZE / 4096)
            .expect("[smp] Failed to allocate AP stack");
        AP_STACKS[i] = stack + AP_STACK_SIZE as u64;
    }

    let pml4_phys = crate::arch::paging::read_cr3();
    ...
    for target_ap in 0..cores_to_start {
        ...
        while read_volatile((base + PD_READY) as *const u32) != AP_READY_MAGIC {
            if cpu::rdtsc().wrapping_sub(start) > AP_TIMEOUT_CYCLES {
                serial::serial_write("[smp] AP timeout, LAPIC ID=");
    ...
    AP_COUNT = started;
    serial::serial_write("[smp] AP bring-up complete: ");
    serial_write_num(online_cpus() as u64);
    serial::serial_write(" CPU(s) online.\n");
}

unsafe fn detect_core_count() -> usize {
    let (_, ebx, _, _) = cpu::cpuid(0x0B, 0);
...
unsafe fn get_ap_lapic_id(ap_idx: usize) -> u32 {
    ...
    for level in 0..32 {
        let (_, ebx, ecx, _) = cpu::cpuid_x2(0x0B, level);
        let shift = ebx & 0xFF;
        if shift == 0 || ecx == 0 {
            break;
        }
