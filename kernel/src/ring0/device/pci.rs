...
pub fn init_ecam(base: u64, end_bus: u8) {
    ...
    // and this one.
    crate::device::serial::serial_write("[pci D1] init_ecam ENTERED\n");

    crate::device::serial::serial_write("[pci] init_ecam: base=0x");
    print_hex(base);
    crate::device::serial::serial_write(" end_bus=");
    print_u32(end_bus as u32);
    crate::device::serial::serial_write("\n");
    crate::device::serial::serial_write("[pci D2] after header log\n");

    if base == 0 || base < 0x1000 {
        crate::device::serial::serial_write("[pci] ECAM base invalid, falling back to IO ports\n");
        unsafe {
            ECAM_BASE = 0;
            ECAM_END_BUS = 0;
            USE_ECAM = false;
    ...
    crate::device::serial::serial_write("[pci D3] base sanity OK\n");
    ...
    crate::device::serial::serial_write("[pci D4] safe_end=");
    print_u32(safe_end as u32);
    crate::device::serial::serial_write(" round_up_2mb=0x");
    ...
    // space without #PF).
    crate::device::serial::serial_write("[pci] Mapping ECAM at 0x");
    print_hex(base);
    crate::device::serial::serial_write(" (");
    print_u32((round_up_2mb / 0x20000) as u32);
    crate::device::serial::serial_write(" x 2 MiB huge pages)\n");
    crate::device::serial::serial_write("[pci D5] about to map_kernel_mmio_huge\n");
    ...
    let result = unsafe {
        crate::memory::paging::map_kernel_mmio_huge(base, base, round_up_2mb as usize)
    };
    crate::device::serial::serial_write("[pci D6] map_kernel_mmio_huge returned\n");
    match result {
        Ok(()) => {
            crate::device::serial::serial_write("[pci] ECAM mapped OK\n");
            unsafe {
                ECAM_BASE = base;
        ...
        Err(e) => {
            crate::device::serial::serial_write("[pci] ECAM map FAILED: ");
            crate::device::serial::serial_write(e);
            crate::device::serial::serial_write(" — falling back to IO ports\n");
            unsafe {
                ECAM_BASE = 0;
            ECAM_END_BUS = 0;
                USE_ECAM = false;
    ...
    crate::device::serial::serial_write("[pci] ECAM initialized .end_bus=");
    print_u32(safe_end as u32);
    crate::device::serial::serial_write(")\n");
    crate::device::serial::serial_write("\n");
}
...
fn print_hex(val: u64) {
    ...
    crate::device::serial::serial_write(core::str::from_utf8(&buf).unwrap_or("0x???"));
}

fn print_u32(val: u32) {
    if val == 0 {
        crate::device::serial::serial_write("0");
        return;
    }
    let mut buf = [0u8; 10];
    let mut i = buf.len();
