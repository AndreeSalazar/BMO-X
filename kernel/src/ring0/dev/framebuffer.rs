...
//!
//! v1.7.5: extraído de `device::gop` (eliminado). El módulo provee:
//! - `Color`: ARGB 32-bit color type with alpha/lerp.
//! - `GopDisplay`: struct describing the GOP framebuffer.
//! - Primitives: put_pixel, get_pixel, fill_rect, draw_line, etc.
//! - Backbuffer: 8.29 MB static in BSS to avoid heap alloc.
        ...
        for py in y..y + h {
            ...
            for px in x..x + w {
                ...
                if in_corner {
                    ...
                    let dist_sq = dx * dx + dy * dy;
                    if dist_sq > radius as u32 * radius as u32 { continue; }
                }
                unsafe {
                    let offset = (py * disp.stride + px) as usize;
                    *disp.base.add(offset) = color.0;
...
pub fn draw_line(x0: i32, y0: i32, x1: i32, y1: i32, color: Color) {
    ...
    let mut err = dx - dy;
    loop {
        if x0 >= 0 && y0 >= 0 { put_pixel(x0 as u32, y0 as u32, color); }
...
fn sqrt_approx(x: f32) -> f32 {
    if x <= 0.0 { return 0.0; }
    let mut guess = x / 2.0;
    for _ in 0..16 { guess = (guess + x / guess) / 2.0; }
    guess
}

fn fract(x: f32) -> f32 { x - floor(x) }
    ...
    for y in min_y..=max_y {
        ...
        for i in 0..points.len() {
            ...
            if (y0 <= y && y1 > y) || (y1 <= y && y0 > y) {
                ...
                if count < 32 { intersections[count] = x; count += 1; }
            }
        }
        for i in 0..count {
            for j in (i + 1)..count {
                if intersections[i] > intersections[j] { intersections.swap(i, j); }
            }
        }
        let mut i = 0;
        while i + 1 < count {
