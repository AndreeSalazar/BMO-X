...
pub fn register_rt(tid: Tid, period: RtPeriod) -> bool {
    unsafe {
        ...
        console::serial_write("[sched] RT thread registered, tid=");
        crate::boot::serial::u64_dec(tid.0 as u64);
        console::serial_write(" period=");
        match period {
            RtPeriod::Audio => console::serial_write("Audio"),
            RtPeriod::Render => console::serial_write("Render"),
            RtPeriod::Physics => console::serial_write("Physics"),
            RtPeriod::Input => console::serial_write("Input"),
            RtPeriod::Network => console::serial_write("Network"),
        }
...
pub fn print_status() {
    ...
    console::serial_write("[sched] RT threads: ");
    u64_dec(unsafe { RT_COUNT as u64 });
    console::serial_write(" util=");
...
fn serial_write_f32(val: f32) {
    ...
    u32_dec(int_part);
    console::serial_write(".");
    if frac_part < 10 { console::serial_write_byte(b'0'); }
    u32_dec(frac_part);
}
