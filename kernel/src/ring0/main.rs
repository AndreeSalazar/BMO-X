...
//! each phase in order. Every phase lives in `crate::boot::phases`
//! and implements `bmo_core::boot::phases::Phase`, which provides both `run`
//! (normal boot) and `self_test` (isolated, non-destructive).
//!
//! Phases are black boxes to main.rs. To add a new phase, add a module
//! under `bmo_core::boot::phases`, implement the `Phase` trait, and add
//! one line to the dispatch below.
...
// ── Ring 0: este archivo vive aquí (módulos privados) ──────────────
mod allocator;
mod arch;
mod boot_info;
mod drivers;
...
// ── BMO Core: windowing + desktop + UI + lang + fs ──────────────────
#[path = "../bmo_core/mod.rs"]
mod bmo_core;

// ── Ring 3: userland (vacío por ahora) ──────────────────────────────
#[path = "../ring3/mod.rs"]
mod ring3;
...
extern "C" fn kernel_main_real(boot_info_ptr: *const fastos_boot_protocol::BootInfo) -> ! {
    ...
    boot::visual::log("boot", "K5a bmo_api::init...", boot::visual::color::OK);
    bmo_core::bmo_api::init();
    boot::visual::log("boot", "K5b bmo_api::init DONE", boot::visual::color::OK);
