...
//!
//! v1.4.0: Implementación inicial con state machine completa, three-way
//! handshake, y send/recv. Sliding window básico.
//!
...
//! - Sin retransmisión timer (asumimos LAN rápida)
//! - Sin congestion control (Reno/CUBIC)
//! - Sin Nagle algorithm
//! - Sin keep-alive
//!
//! Estas features se añaden en v1.5+.
...
/// TCP header (20 bytes mínimo, sin opciones).
#[repr(C, packed)]
#[derive(Clone, Copy)]
...
// ── TCP Flags (bits en data_offset_flags después del offset) ──────────
...
/// Extrae data offset (4 bits altos del primer byte) — mide header en palabras de 32 bits.
...
/// Tabla de sockets TCP (estática, capacidad fija).
...
/// v1.4.0: usamos un counter atómico. En producción debería usar
/// un PRNG o timestamp.
static ISN_COUNTER: AtomicU64 = AtomicU64::new(0xDEAD_BEEF);

/// Genera un ISN determinístico pero único por conexión.
fn next_isn() -> u32 {
...
/// Inicia conexión (cliente): envía SYN.
...
/// Envía datos por un socket Established.
...
fn ephemeral_port() -> u16 {
    // Para v1.4.0: usar puerto fijo. En producción: pool dinámico.
...
// silences bx_u32 unused import warning — used in future versions
#[allow(dead_code)]
