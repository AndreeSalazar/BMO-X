...
pub fn register_symbol(lib: &'static str, name: &'static str, addr: u64, flags: u32) {
    unsafe {
        if SYMBOL_COUNT >= MAX_SYMBOLS {
            crate::diag::warn("bef", "runtime symbol table full");
            return;
        }
...
pub fn resolve_imports(
    ...
    for entry in import_table.entries {
        ...
        if addr != 0 {
            ...
            unresolved += 1;
            crate::diag::warn_u64("bef", "unresolved import", entry.binding_offset);
        }
    }

    if unresolved > 0 {
        crate::diag::warn_u64("bef", "total unresolved imports", unresolved as u64);
    }
...
pub fn init() {
    ...
    crate::diag::info_u64("bef", "runtime symbol table initialized", symbol_count() as u64);
}
