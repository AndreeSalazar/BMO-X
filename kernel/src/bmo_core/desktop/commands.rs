...
pub fn nexo_test_compile() {
    ...
    crate::cabina::info("bmo", "Compiling test program");
    match bmo::compile(source) {
        Ok(bytes) => {
            crate::cabina::info("bmo", "Compilation succeeded");
            crate::bmo_core::diag::info_u64("bmo", "Generated bytes", bytes.len() as u64);
            crate::bmo_core::diag::info_u64("bmo", "First byte", bytes.first().copied().unwrap_or(0) as u64);
        }
        Err(_e) => {
            crate::cabina::warn("bmo", "Compilation failed");
        }
    }
}
