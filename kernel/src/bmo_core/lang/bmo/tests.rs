...
//!
//! Tests the complete pipeline: ÑEXO source → BMO assembly syntax (legacy) → x86_64 bytes.
...
use super::compile;
use super::compile_c;
use crate::bmo_core::lang::bmo::traductor::Traductor;
use crate::bmo_core::lang::bmo::parser::Parser;
use crate::bmo_core::lang::bmo::parser::ast::Ast;

/// Compile ÑEXO source to BMO assembly syntax (legacy) text (for inspection).
...
pub fn test_nexo_compile_struct_layout() -> Result<(), String> {
    // Verify BMO assembly syntax (legacy) type layout for a 3-field struct.
    let src = b"
...
pub fn test_pipeline_c_nexo_bmoasm_x86_64() -> Result<(), String> {
    // Full end-to-end: C source → ÑEXO → BMO assembly syntax (legacy) → x86_64 bytes
    let c_source = b"
        int factorial(int n) {
            if (n <= 1) {
                return 1;
...
pub fn test_pipeline_nexo_bmoasm_x86_64() -> Result<(), String> {
    // Full e2e: ÑEXO source → BMO assembly syntax (legacy) → x86_64
    let nexo_source = b"
