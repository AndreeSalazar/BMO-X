//! Tests integrados del `bmo_core::ring3_gateway`.
//!
//! Valida que la única puerta Ring 0 → BMO Core funciona correctamente.

#![allow(dead_code)]

use crate::bmo_core::ring3_gateway;
...
fn test_out_of_range_low() -> TestResult {
    let total_before = ring3_gateway::total();
    let r = ring3_gateway::enter(0x050, 0, 0, 0, 0, 0, 0);
    if r == 0xFFFF_FFFF_FFFF_FFFF || r > 0x1000 {
        pass("out_of_range_low", &alloc::format!("nr=0x050 rejected, total={}->{}", total_before, ring3_gateway::total()))
    } else {
...
fn test_out_of_range_high() -> TestResult {
    let r = ring3_gateway::enter(0x300, 0, 0, 0, 0, 0, 0);
    if r == 0xFFFF_FFFF_FFFF_FFFF || r > 0x1000 {
...
fn test_valid_syscall_time() -> TestResult {
    let r = ring3_gateway::enter(syscalls::NR_TIME_NOW_NS as u16, 0, 0, 0, 0, 0, 0);
...
fn test_valid_syscall_proc_get_pid() -> TestResult {
    let r = ring3_gateway::enter(syscalls::NR_PROC_GET_PID as u16, 0, 0, 0, 0, 0, 0);
...
fn test_valid_syscall_audio_beep() -> TestResult {
    let r = ring3_gateway::enter(syscalls::NR_AUDIO_BEEP as u16, 1000, 50, 0, 0, 0, 0);
    // OK = 0.
    if r == 0 {
...
fn test_valid_syscall_debug_print() -> TestResult {
    ...
    let r = ring3_gateway::enter(syscalls::NR_DEBUG_PRINT as u16, 0, 2, 0, 0, 0, 0);
...
fn test_stats_increment() -> TestResult {
    let t0 = ring3_gateway::total();
    let a0 = ring3_gateway::allowed();
    let d0 = ring3_gateway::denied();
    let u0 = ring3_gateway::unknown();

    ring3_gateway::enter(syscalls::NR_TIME_NOW_NS as u16, 0, 0, 0, 0, 0, 0);
    ring3_gateway::enter(syscalls::NR_AUDIO_BEEP as u16, 500, 10, 0, 0, 0, 0);
    ring3_gateway::enter(0x050, 0, 0, 0, 0, 0, 0);
    ring3_gateway::enter(0x300, 0, 0, 0, 0, 0, 0);

    let t1 = ring3_gateway::total();
    let a1 = ring3_gateway::allowed();
    let d1 = ring3_gateway::denied();
    let u1 = ring3_gateway::unknown();
