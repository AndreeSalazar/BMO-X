...
use crate::bmo_core::bef::header::{BefHeader, BEF_MAGIC};
use crate::bmo_core::bef::sections::{SectionTable, SectionKind, SectionEntry};
use crate::bmo_core::bef::signing::{SignatureHeader, SectionHash};
...
pub fn load(bytes: &[u8]) -> Result<Image, LoadError> {
    ...
    if hdr.arch != crate::bmo_core::bef::header::BefArch::X86_64 as u8 {
    ...
    if let Some(imports_entry) = table.find(SectionKind::Imports) {
        ...
        if import_start + import_size <= bytes.len() && import_size >= 4 {
            ...
            if let Ok(import_table) = crate::bmo_core::bef::imports::ImportTable::parse(entry_data, count) {
...
fn verify_section_hashes(bytes: &[u8], table: &SectionTable) -> Result<(), LoadError> {
    ...
    for i in 0..hash_count {
        ...
        let computed = crate::bmo_core::bef::signing::blake3_256(section_bytes);
