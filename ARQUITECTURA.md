...
Everything else is a **subsyscall**: the pair `(handle kind × operation)`,
...
(8× `INVOKE·CONSOLE_WRITE` + 1× `INVOKE·EXIT`). Surface intact.
...
- **Unified UEFI boot** → `BOOTX64.EFI` with `s1_cpu` + `s2_mem` + kernel
  embedded → GOP 1920×1080. Zero dependence on the firmware's FAT reader
...
- **Capability Engine**: 16 processes × 64 slots, handles with generation
...
- **USB keyboard** (xHCI + HID): es-latam / es-españa / us layouts switchable
...
- **Per-CPU pagesets** sit above both: each core caches 16 pages × 5 orders
...
- **GPU**: UEFI GOP framebuffer (1920×1080 BGR, backbuffer)
...
> ⚠️ The NVMe in this machine holds the owner's Windows. The kernel asks for the
