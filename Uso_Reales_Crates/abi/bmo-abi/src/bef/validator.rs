...
impl ValidationResult {
    pub fn new() -> Self {
        Self { issues: Vec::new(), is_valid: true, is_loaded: false }
    }
...
pub fn validate(bytes: &[u8]) -> ValidationResult {
    ...
    if bytes.len() < BefHeader::SIZE {
        r.error(format!("file too small: {} bytes, need at least {} for header", bytes.len(), BefHeader::SIZE));
        return r;
    }
    ...
    if !r.is_valid && r.issues.iter().any(|i| matches!(i.severity, IssueSeverity::Error) && i.message.contains("magic")) {
        return r;
    }
    ...
    if table_offset + table_size > bytes.len() {
        r.error(format!("section table out of bounds: offset {} size {} > file len {}", table_offset, table_size, bytes.len()));
        return r;
    }
    ...
    for i in 0..header.section_count as usize {
        ...
        let e: SectionEntry = unsafe {
            core::ptr::read_unaligned(entry_raw[off..].as_ptr() as *const SectionEntry)
        };
        entries.push(e);
    }
    ...
    let code_idx = entries.iter().position(|e| e.kind == SectionKind::Code as u8);
    ...
    for (i, entry) in entries.iter().enumerate() {
        let kind = match entry.kind() {
            Some(k) => k,
            None => { r.warn_at(i, format!("unknown section kind {:#04x}", entry.kind)); continue; }
        };
        match kind {
            SectionKind::Code => {}
            ...
            SectionKind::Unwind | SectionKind::Debug => validate_optional_section(entry, bytes, i, &mut r),
            _ => {}
        }
...
fn validate_header(header: &BefHeader, bytes: &[u8], r: &mut ValidationResult) {
    if header.magic != BEF_MAGIC {
        ...
        r.error(format!("bad magic: expected BEF1 ({:#x}), got {:#x}", BEF_MAGIC, actual));
        return;
    }
    ...
    if header.flags & !(BefFlags::all().bits()) != 0 {
        r.warn(format!("unknown header flags: {:#x}", header.flags & !BefFlags::all().bits()));
    }
}
...
fn validate_section_table(entries: &[SectionEntry], bytes: &[u8], r: &mut ValidationResult) {
    ...
    for (i, entry) in entries.iter().enumerate() {
        ...
        if kind >= seen.len() {
            r.error_at(i, format!("section kind {:#04x} out of valid range", entry.kind));
            continue;
        }
        ...
        if entry.kind == SectionKind::Bss as u8 {
            ...
            if entry.file_size > 0 {
                r.warn_at(i, "BSS section should have file_size = 0 (data is zero-filled at load)");
            }
        ...
        if entry.file_size > 0 {
            if file_start >= bytes.len() {
                r.error_at(i, format!("section file offset {:#x} beyond file end", file_start));
                continue;
            }
            if file_end > bytes.len() {
                r.error_at(i, format!(
                    "section file range out of bounds: [{:#x}, {:#x}) > file len {}",
                    file_start, file_end, bytes.len()
                ));
                continue;
            }
        }
        ...
        if entry.alignment > 0 && (entry.alignment & (entry.alignment - 1)) != 0 {
            r.warn_at(i, format!("alignment {} is not a power of 2", entry.alignment));
        }
    ...
    for i in 0..entries.len() {
        let a = &entries[i];
        if a.kind == SectionKind::Bss as u8 || a.file_size == 0 { continue; }
        let a_start = a.file_offset as usize;
        let a_end = a_start + a.file_size as usize;
        for j in (i + 1)..entries.len() {
            let b = &entries[j];
            if b.kind == SectionKind::Bss as u8 || b.file_size == 0 { continue; }
...
fn validate_import_section(entry: &SectionEntry, bytes: &[u8], idx: usize, r: &mut ValidationResult) {
    let data = match get_section_data(entry, bytes) {
        Some(d) => d,
        None => { r.error_at(idx, "Imports section data unavailable"); return; }
    };
    let entry_sz = core::mem::size_of::<ImportEntry>();
    if data.len() < entry_sz {
        r.error_at(idx, format!("Imports section too small: {} bytes, need at least {}", data.len(), entry_sz));
        return;
    }
    ...
    for (i, imp) in (0..count).map(|i| unsafe { &*raw.add(i) }).enumerate() {
        ...
        if string_start + lib_off + 2 > data.len() {
            r.error_at(idx, format!("import[{}]: library_name_off {:#x} out of strings range", i, lib_off));
        } else {
            let slen = u16::from_le_bytes(strings[lib_off..lib_off + 2].try_into().unwrap_or([0; 2])) as usize;
            if string_start + lib_off + 2 + slen > data.len() {
                r.error_at(idx, format!("import[{}]: library name length {} exceeds strings", i, slen));
            }
        }
        if string_start + sym_off + 2 > data.len() {
            r.error_at(idx, format!("import[{}]: symbol_name_off {:#x} out of strings range", i, sym_off));
        }
        if imp.binding_offset > 0 && imp.binding_offset < u64::MAX {
            ...
            if imp.binding_offset > bytes.len() as u64 {
                r.warn_at(idx, format!("import[{}]: binding_offset {:#x} is beyond file size", i, imp.binding_offset));
            }
        }
    }
}

fn validate_export_section(entry: &SectionEntry, bytes: &[u8], idx: usize, r: &mut ValidationResult) {
    let data = match get_section_data(entry, bytes) {
        Some(d) => d,
        None => { r.error_at(idx, "Exports section data unavailable"); return; }
    };
    let entry_sz = core::mem::size_of::<ExportEntry>();
    if data.len() < entry_sz {
        r.error_at(idx, format!("Exports section too small: {} bytes", data.len()));
        return;
    }
    ...
    for i in 0..count {
        ...
        if abs_off + 2 > data.len() {
            r.error_at(idx, format!("export[{}]: symbol_name_off {:#x} out of strings range", i, off));
        } else {
            let slen = u16::from_le_bytes(data[abs_off..abs_off + 2].try_into().unwrap_or([0; 2])) as usize;
            if abs_off + 2 + slen > data.len() {
                r.error_at(idx, format!("export[{}]: symbol name length {} exceeds strings", i, slen));
            }
        }
    }
}
...
fn validate_symbol_section(
    ...
    let data = match get_section_data(entry, bytes) {
        Some(d) => d,
        None => { r.error_at(idx, "Symbols section data unavailable"); return; }
    };
    let entry_sz = Symbol::SIZE;
    if data.len() < entry_sz {
        r.error_at(idx, format!("Symbols section too small: {} bytes", data.len()));
        return;
    }
    ...
    for i in 0..count {
        ...
        let abs_off = string_start + off;
        if abs_off + 2 > data.len() {
            r.error_at(idx, format!("symbol[{}]: name_off {:#x} out of strings range", i, off));
        }
        if sym.kind() == None {
            r.warn_at(idx, format!("symbol[{}]: unknown symbol kind {:#04x}", i, sym.kind));
        }
        if sym.binding() == None {
            r.warn_at(idx, format!("symbol[{}]: unknown binding {:#04x}", i, sym.binding));
        }
        if !matches!(sym.visibility, 0x00..=0x03) {
            r.warn_at(idx, format!("symbol[{}]: unknown visibility {:#04x}", i, sym.visibility));
        }
...
fn validate_reloc_section(
    ...
    let data = match get_section_data(entry, bytes) {
        Some(d) => d,
        None => { r.error_at(idx, "Relocs section data unavailable"); return; }
    };
    ...
    for i in 0..count {
        ...
        if ts >= target_map.len() {
            r.error_at(idx, format!("reloc[{}]: target_section {} out of range (0..2)", i, ts));
        } else {
        ...
        let syms = sections.iter().find(|s| s.kind == SectionKind::Symbols as u8);
        let imps = sections.iter().find(|s| s.kind == SectionKind::Imports as u8);
...
fn validate_bss_section(entry: &SectionEntry, idx: usize, r: &mut ValidationResult) {
    ...
    if entry.flags & SectionFlags::WRITE.bits() == 0 {
        r.warn_at(idx, "BSS section missing WRITE flag (cannot zero-fill without write permission)");
    }
}
...
fn validate_signature(entry: &SectionEntry, bytes: &[u8], r: &mut ValidationResult) {
    let data = match get_section_data(entry, bytes) {
        Some(d) => d,
        None => { r.error("Signature section data unavailable"); return; }
    };
    let hdr_sz = core::mem::size_of::<SignatureHeader>();
    let hash_sz = core::mem::size_of::<SectionHash>();
    if data.len() < hdr_sz {
        r.error(format!("Signature section too small for header: {} < {}", data.len(), hdr_sz));
        return;
    }
    ...
    if data.len() < needed {
        r.error(format!(
            ...
            data.len(), needed, hash_count
        ));
    }
    if sig_header.sig_algo > 1 {
        r.warn(format!("unknown signature algorithm: {}", sig_header.sig_algo));
    }
}

fn validate_manifest_section(entry: &SectionEntry, bytes: &[u8], idx: usize, r: &mut ValidationResult) {
    let data = match get_section_data(entry, bytes) {
        Some(d) => d,
        None => { r.error_at(idx, "Manifest section data unavailable"); return; }
    };
    if data.is_empty() {
...
fn validate_tls_section(entry: &SectionEntry, bytes: &[u8], idx: usize, r: &mut ValidationResult) {
    let data = match get_section_data(entry, bytes) {
        Some(d) => d,
        None => { r.error_at(idx, "TLS section data unavailable"); return; }
    };
    if data.len() < 16 {
        r.warn_at(idx, format!("TLS section very small: {} bytes (template header is 16)", data.len()));
    }
}

fn validate_optional_section(entry: &SectionEntry, bytes: &[u8], idx: usize, r: &mut ValidationResult) {
    let data = match get_section_data(entry, bytes) {
        Some(d) => d,
        None => { return; }
    };
...
fn validate_section_flags(entry: &SectionEntry, kind: SectionKind, idx: usize, r: &mut ValidationResult) {
...
mod tests {
    ...
    fn validate_duplicate_sections() {
        ...
        let bytes = b.build().unwrap();
        let r = validate(&bytes);
        assert!(r.is_valid, "duplicate Code is a warning, not error: {:?}", r.issues);
        let has_warn = r.issues.iter().any(|i| matches!(i.severity, IssueSeverity::Warning) && i.message.contains("duplicate"));
    ...
    fn validate_section_overlap() {
        ...
        let sec0_file_off = u64::from_le_bytes(
            corrupted[table_start + 8..table_start + 16].try_into().unwrap()
        );
    ...
    fn validate_section_flags_missing_exec() {
        ...
        assert!(has_flag_err, "should flag missing EXEC on Code: {:?}", r.issues);
    }
    ...
    fn validate_manifest_utf8() {
        ...
        assert!(has_utf8_warn, "should warn about non-UTF-8 manifest: {:?}", r.issues);
    }
}
