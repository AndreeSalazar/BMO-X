...
#[test]
fn bef_build_validate_y_la_puerta_del_kernel() {
    ...
    // ** Por la PUERTA que usa el kernel (2026-09-19), no por el cargador v1
    // de este crate, que se borro: lo que se prueba es lo que el Ryzen hace.
    let loaded = bmo_bex_gate::revisar(&bytes, bytes.len()).unwrap();
    assert!(loaded.buscar(bmo_bex_gate::CODE).is_some());
