...
impl BmoAllocator for BmoGlobalAllocator {
    fn alloc(&mut self, size: bx_usize, align: bx_usize) -> BmoAllocResult {
        let layout = match core::alloc::Layout::from_size_align(size as usize, align as usize) {
            Ok(l) => l,
            Err(_) => {
                return BmoAllocResult {
                    status: BmoStatus::err(crate::bmo_abi::error_code::INVALID_ARGUMENT),
        ...
        if ptr.is_null() {
            BmoAllocResult {
                status: BmoStatus::err(crate::bmo_abi::error_code::OUT_OF_MEMORY),
                ptr: core::ptr::null_mut(),
            }
        } else {
            BmoAllocResult {
