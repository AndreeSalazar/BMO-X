...
//   7. APs load GDT/IDT from shared data, signal online, halt
//   8. BSP waits for all APs to come online (atomic counter at 0x7008)
//
// Memory layout:
//   0x7000-0x7FFF: PML4 (4KB) + online counter at 0x7FF8
//   0x8000-0x80FF: Trampoline (256 bytes)
//   0x8100-0x81FF: Shared BSP↔AP data
//   0x8200-...:    Per-AP stacks (4KB each, max 16 APs = 64KB)
...
_smp_trampoline_start:
    ...
    mov ss, ax
    mov sp, 0x8F00
    /* Use bx to hold offset, then lgdt [cs:bx] via segment override */
    mov bx, 0x58
    ...
    or al, 1
    mov cr0, eax
    /* Far jump via stack: push offset then push segment, then retf */
    push 8
    push offset pmode32
    retf
.code32
pmode32:
    ...
    or ax, 0x20
    mov cr4, eax
    mov eax, [0x8100]      /* PML4 address from shared data */
    mov cr3, eax
    mov ecx, 0xC0000080
    rdmsr
    or ax, 0x901          /* LME | NXE */
    ...
    mov cr0, eax
    lgdt [0x8058]
    /* Far jump to 64-bit code via stack */
    push 0x18
    push offset lmode64
    retf
.code64
lmode64:
    ...
    mov gs, ax
    mov ss, ax
    mov rsp, [0x8108]      /* Stack top from shared data */
    mov rax, [0x8110]      /* AP entry point from shared data */
    jmp rax

/* NOP padding to reach offset 0x58 (GDT pointer location) */
.byte 0x90, 0x90, 0x90, 0x90
.byte 0x90, 0x90, 0x90, 0x90
.byte 0x90, 0x90, 0x90, 0x90
.byte 0x90, 0x90, 0x90, 0x90

/* GDT pointer at offset 0x58 */
smp_gdt_ptr:
    .word 23
    .long 0x8060
...
unsafe extern "C" {
    static _smp_trampoline_start: u8;
}

// ── LAPIC (Local APIC) ───────────────────────────────────────────

const LAPIC_REG_ID: u32       = 0x020;
const LAPIC_REG_SIVR: u32     = 0x0F0;
const LAPIC_REG_TPR: u32      = 0x080;
const LAPIC_REG_ICR_LO: u32   = 0x300;
const LAPIC_REG_ICR_HI: u32   = 0x310;
...
unsafe fn lapic_id() -> u32 {
    (lapic_read(LAPIC_REG_ID) >> 24) & 0xFF
}

unsafe fn lapic_init() {
    // Enable LAPIC in MSR 0x1B (set bit 11)
    let lo: u32; let hi: u32;
    asm!("rdmsr", in("ecx") 0x1Bu32, out("eax") lo, out("edx") hi);
    asm!("wrmsr", in("ecx") 0x1Bu32, in("eax") lo | (1 << 11), in("edx") hi);
    // SIVR: enable + spurious vector 0xFF
    lapic_write(LAPIC_REG_SIVR, 0x100 | 0xFF);
    // TPR: accept all interrupts
    lapic_write(LAPIC_REG_TPR, 0);
}

unsafe fn send_init_ipi(apic_id: u32) {
    // ICR: INIT delivery, assert, all-excluding-self, edge-triggered
    let icr = 0x000C4500u32 | ((apic_id & 0xFF) << 24);
    lapic_write(LAPIC_REG_ICR_HI, (apic_id >> 8) & 0xFF);
    lapic_write(LAPIC_REG_ICR_LO, icr);
    // Wait for delivery
    while lapic_read(LAPIC_REG_ICR_LO) & (1 << 12) != 0 {}
}

unsafe fn send_sipi(apic_id: u32, vector: u8) {
    // ICR: Start Up delivery, assert, all-excluding-self, edge-triggered
...
unsafe fn setup_smp_pml4() {
    ...
    // PML4[0] = PDPT
    core::ptr::write_volatile(pml4, 0x7103); // PRESENT | WRITABLE
    // PDPT[0] = PD
    core::ptr::write_volatile(pdpt, 0x7203);
    // PD[0..2047] = 2MB huge pages for 0..4GB
...
unsafe fn copy_trampoline() {
    let src = core::ptr::addr_of!(_smp_trampoline_start) as *const u8;
...
// ── AP entry point (64-bit, runs on each AP) ─────────────────────

#[unsafe(no_mangle)]
#[unsafe(link_section = ".text.ap_entry")]
#[unsafe(naked)]
pub unsafe extern "C" fn ap_entry() {
    // We're in 64-bit long mode with BSP's PML4 active.
    // Shared data at 0x8100:
    //   +8:  stack top
    //   +16: this function (already jumped, ignore)
    //   0x8138: IDT pointer (10 bytes: limit + base)
    //   0x8140: online counter (u32, atomic)
    naked_asm!(
        // Reload GDT (trampoline's GDT at 0x8058)
        "lgdt [0x8058]",
        // Load IDT from BSP
        "lidt [0x8138]",
        // Stack was set by trampoline; ensure it's valid
        // Segment registers: largely ignored in 64-bit mode
        "xor ax, ax",
        "mov ds, ax",
        "mov es, ax",
        "mov fs, ax",
        "mov gs, ax",
        "mov ss, ax",
        // Signal online: atomic increment of counter at 0x8140
        "lock inc dword ptr [0x8140]",
...
// ── SMP startup (BSP sends IPIs) ────────────────────────────────

unsafe fn smp_startup() {
    ser_print!("\n[s1_cpu] === AMD SMP STARTUP (Zen 3) ===\n");

    // 1. Build minimal PML4
    setup_smp_pml4();
    ser_print!("[s1_cpu] PML4 at 0x7000 (identity 0..4GB)\n");

    // 2. Copy trampoline to physical 0x8000
    copy_trampoline();
    ser_print!("[s1_cpu] trampoline at 0x8000\n");

    // 3. Write IDT pointer to 0x8138 (APs will load it)
    let idt_ptr_addr = 0x8138 as *mut u8;
    let idt_limit = (core::mem::size_of::<[IdtEntry; 256]>() - 1) as u16;
    core::ptr::write_volatile(idt_ptr_addr as *mut u16, idt_limit);
    let idt_base = core::ptr::addr_of!(IDT) as u64;
    core::ptr::write_volatile(idt_ptr_addr.add(2) as *mut u64, idt_base);

    // 4. Initialize online counter at 0x7FF8 (within PML4 page)
    ...
    // 7. For each possible APIC ID (0..15), send INIT+SIPI
    //    APs respond by jumping to trampoline → 64-bit → ap_entry → online
    let cpu = unsafe { &CPU };
    ...
    for apic_id in 0..16u32 {
        ...
        // Set up per-AP data in shared memory at 0x8100
        let shared = 0x8100 as *mut u64;
        core::ptr::write_volatile(shared.add(0), 0x7000); // PML4 address
        let stack_top = 0x8200u64 + (apic_id as u64) * 0x1000 + 0x1000;
        core::ptr::write_volatile(shared.add(1), stack_top);
        let ap_entry_addr = ap_entry as *const () as u64;
        core::ptr::write_volatile(shared.add(2), ap_entry_addr);
        ...
        delay_ms(10);
        // SIPI #1 (vector=8 → address 0x8000)
        send_sipi(apic_id, 8);
        delay_ms(1);
        // SIPI #2 (per Intel SDM §8.4, recommended for reliable startup)
        send_sipi(apic_id, 8);
        delay_ms(1);
    }

    // 8. Wait for all APs to come online
    let expected_aps = num_threads - 1;
    ser_print!("[s1_cpu] waiting for ");
    ser_dec!(expected_aps as usize);
    ser_print!(" APs...\n");
    let mut online: u32 = 0;
    let mut timeout: u32 = 1000; // 1 second
    while online < expected_aps && timeout > 0 {
        online = core::ptr::read_volatile(counter);
