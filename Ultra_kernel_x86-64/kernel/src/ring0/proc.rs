...
/// Snapshot of the fabricated Ring 3 frame (rip, cs, ss, rsp, rflags) so the
/// exact values `iretq` will load can be surfaced on screen for debugging the
/// first CPL3 entry. Populated by `spawn_init`.
pub static mut FRAME_SNAPSHOT: [u64; 5] = [0; 5];

/// Direct test of kernel-half sharing: [value read under kernel CR3, value
/// read at the same high-mem address under the user CR3]. If they differ, the
/// new address space does not share the physmap and the fabricated context is
/// invisible after the scheduler switches CR3 — which is exactly the #GP.
pub static mut CR3_TEST: [u64; 2] = [0; 2];

/// Embedded "hola mundo" Ring 3 program (generated by `toolchain/hello-bex`).
...
pub fn spawn_init(ctx: &BootContext) -> Option<u32> {
    ...
    // Read the frame back exactly as iretq will consume it (gpr_base sits
    // 552 bytes above the returned fxsave_base) so its values can be shown.
    unsafe {
        let frame = &*((context + 552) as *const trap::TrapFrame);
        FRAME_SNAPSHOT = [frame.rip, frame.cs, frame.ss, frame.rsp, frame.rflags];
    }

    // Decisive sharing test: read the context back-pointer (fxsave_base+512)
    // under BOTH CR3s. The executing code (identity 0..1 GiB) and this stack
    // are shared, so switching CR3 here and back is safe. If the user-CR3 read
    // returns 0 while the kernel-CR3 read is valid, the high-mem context is
    // not shared — that is the root cause of the iretq #GP.
    unsafe {
        let backptr = (context + 512) as *const u64;
        let k_val = backptr.read_volatile();
        let kcr3 = vmm::read_cr3();
        vmm::switch_to(aspace);
        let u_val = backptr.read_volatile();
        vmm::switch_to(kcr3);
        CR3_TEST = [k_val, u_val];
    }
