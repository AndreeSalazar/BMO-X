...
/// `TALLYING` -- contar apariciones. En banca, lo mas corriente es contar
...
fn inspect_tallying_cuenta_las_veces() {
    ...
    // "AB CD EF" son ocho letras; el campo es de diez, asi que hay dos
    // espacios dentro y dos de relleno: cuatro.
    assert_eq!(run_cobol(&src), "4\n");
}

/// * `ALL` y `LEADING` NO son lo mismo, y sobre un importe es otro numero.
/// Esta es la razon por la que hay dos formas y no una con una opcion.
#[test]
...
/// anterior.
///
/// * Este test cazo un fallo que no avisaba: la palabra `SIZE` de
/// `DELIMITED BY SIZE` se colaba como si fuera un campo mas, y sus dos
/// primeras letras acababan escritas DENTRO del destino. Compilaba, no
/// decia nada, y metia basura en un registro.
#[test]
...
/// Lo que no se compila se dice con su motivo, y el motivo explica **que
/// pasaria** si se aceptara a medias.
#[test]
