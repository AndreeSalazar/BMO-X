...
pub fn parse_statement(c: &mut Cursor) -> Result<CobolStatement, CobolError> {
    ...
    let st = match kw.as_str() {
        ...
        "ADD" => {
            ...
            crate::ast::CobolStatement::Add(v, operand(c, line, "destino")?, crate::ast::Aritmetica::default())
        }
        "SUBTRACT" => {
            let v = operand(c, line, "valor")?;
            if !c.eat_kw("FROM") {
            ...
            CobolStatement::Subtract(v, operand(c, line, "destino")?, crate::ast::Aritmetica::default())
        }
        "MULTIPLY" => {
            let v = operand(c, line, "valor")?;
            if !c.eat_kw("BY") {
            ...
            CobolStatement::Multiply(v, operand(c, line, "destino")?, crate::ast::Aritmetica::default())
        }
        "DIVIDE" => {
            let v = operand(c, line, "valor")?;
            if !c.eat_kw("BY") {
            ...
            CobolStatement::Divide(v, operand(c, line, "destino")?, crate::ast::Aritmetica::default())
        }
        "STOP" => {
            c.eat_kw("RUN");
...
mod tests {
    ...
    fn parses_core_statements_from_tokens() {
        ...
        assert_eq!(sts[2], CobolStatement::Add("3.20".into(), "SALDO".into(), crate::ast::Aritmetica::default()));
    ...
    fn multiline_and_decimal_period_disambiguation() {
        ...
        let sts = parse_statements(src).unwrap();
        assert_eq!(sts[0], CobolStatement::Multiply("3".into(), "SALDO".into(), crate::ast::Aritmetica::default()));
        assert_eq!(sts[1], CobolStatement::Divide("4".into(), "SALDO".into(), crate::ast::Aritmetica::default()));
    }
