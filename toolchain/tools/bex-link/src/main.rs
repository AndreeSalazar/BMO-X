//! `bex-link` — el eslabón que faltaba entre Rust y BMO.
//!
//! Hasta ahora **no había ninguna forma de convertir un crate de Rust en un
//! programa que BMO pudiera admitir**. Todos los `.bex` del sistema salían de
//! emisores de bytes x86 escritos a mano o de los compiladores propios de C y
//! COBOL. Por eso `Ultra_userspace/` llevaba meses lleno de stubs vacíos: no
//! estaban sin escribir por descuido, es que no existía tubería que los
...
//!
//! El kernel **decide dónde va cada sección**: las coloca secuencialmente
//! desde `USER_IMAGE_BASE`, respetando la alineación que cada una declara y
//! avanzando por páginas enteras (ver `proc.rs`). No hay reubicación: lo que
//! el enlazador escribió como dirección absoluta tiene que caer donde el
//! kernel va a mapearlo, o el programa salta al vacío.
//!
//! Por eso hay un guion de enlazado (`Ultra_userspace/userland/link.ld`) que
//! reproduce exactamente esa colocación —cada sección en su frontera de 4 KiB,
//! en este orden— y por eso aquí **todas las secciones declaran alineación
//! 4096**. Los dos lados calculan la misma dirección porque siguen la misma
//! regla, no porque coincidan por suerte.
//!
//! Si eso se rompe, se rompe en silencio: el programa carga, salta y muere en
//! Ring 3 con un `#UD` o un `#PF` a una dirección redonda. Al final del
//! proceso esta herramienta imprime el mapa que ha calculado, para que ese
//! número se pueda comparar con el que diga el kernel.
//!
//! ## Lo que NO hace
//!
//! No reubica, no enlaza dinámicamente, no resuelve imports y no firma. El
//! `.bex` sale sin firma; quien la quiera la pone después con las herramientas
...
/// Lo que hace falta de un ELF64. Nada más que esto.
struct Elf {
    entry: u64,
...
fn leer_elf(b: &[u8]) -> Result<Elf, String> {
    ...
    // La tabla de nombres, para poder buscar por `.text` en vez de por índice.
...
fn main() {
    ...
    // La dirección que el kernel irá calculando, para poder comprobar que
    // coincide con la que el enlazador escribió. Empieza en USER_IMAGE_BASE.
    ...
    for nombre in ORDEN {
        ...
        // El kernel alinea el cursor y luego avanza por páginas enteras.
        ...
        let mut seccion = if s.tipo == SHT_NOBITS {
            // `.bss` no aporta bytes: el kernel pone la página a cero.
        ...
        // TODAS a 4096: es lo que hace predecible la colocación del kernel.
    ...
    // El kernel lo lee como desplazamiento DENTRO de la sección de código.
