...
pub fn init_msr_common(syscall_entry: u64, bsp_apic_id: u32) {
    unsafe {
        ...
        crate::serial_write("[msr] IA32_EFER: ");
        crate::serial_write_u64(efer_val, 16);
        crate::serial_write("\n");

        // ── IA32_STAR: kernel/user code/stack selectors ────────────
        wrmsr(MSR_IA32_STAR, STAR_VALUE);
        crate::serial_write("[msr] IA32_STAR: ");
        crate::serial_write_u64(STAR_VALUE, 16);
        crate::serial_write("\n");

        // ── IA32_LSTAR: 64-bit SYSCALL entry ───────────────────────
        wrmsr(MSR_IA32_LSTAR, syscall_entry);
        crate::serial_write("[msr] IA32_LSTAR: ");
        crate::serial_write_u64(syscall_entry, 16);
        crate::serial_write("\n");
    ...
    crate::serial_write("[msr] all common MSRs initialized\n");
}
