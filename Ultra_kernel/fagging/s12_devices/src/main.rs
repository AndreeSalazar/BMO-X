...
extern "C" {
    fn kernel_entry(ctx: *mut boot_context::BootContext) -> !;
}
...
fn pci_scan(ctx: &mut boot_context::BootContext, d: &Devs) {
    ...
    ctx.pci_count = count;
    serial_puts("[s12 pci] found "); serial_dec(count as usize); serial_puts(" devices\n");
}
...
unsafe fn ioapic_init(base: u64) {
    ...
    let max = (ver >> 16) & 0xFF;
    for i in 0..=max {
        v.add(0x10 / 4 + i * 2 + 1).write_volatile(0x00010000);
        v.add(0x10 / 4 + i * 2).write_volatile(0);
    }
