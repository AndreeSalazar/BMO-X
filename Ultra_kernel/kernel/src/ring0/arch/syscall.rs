...
fn check_capability(required: u32) -> bool {
    if let Some(task) = crate::ring0::proc::task::current() {
        if let Some(proc) = crate::ring0::proc::process::get_process(task.pid) {
...
fn sys_yield(_nr: u64, _a0: u64, _a1: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64) -> u64 {
    crate::ring0::proc::yield_now();
    0
}

fn sys_task_alloc(_nr: u64, a0: u64, a1: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64) -> u64 {
    match crate::ring0::proc::task::alloc(
        crate::ring0::proc::process::Pid(a0 as u32),
        crate::ring0::proc::Priority::Interactive,
    ) {
        Some(thr) => {
            thr.regs = crate::ring0::proc::task::SavedRegs::new_user(a1, 0);
            thr.state = crate::ring0::proc::task::State::Ready;
            thr.tid.0 as u64
        }
        None => u64::MAX,
    }
...
fn sys_mmap(_nr: u64, cr3: u64, virt: u64, phys: u64, pages: u64, flags: u64, _a5: u64) -> u64 {
    match unsafe { crate::ring0::mm::vmm::map_user_range(cr3, virt, phys, pages as usize, flags) } {
        Ok(()) => 0,
        Err(_) => u64::MAX,
    }
}
...
fn sys_nanosleep(_nr: u64, a0: u64, _a1: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64) -> u64 {
    ...
    let tsc_freq = crate::ring0::cpu::tsc_per_sec();
    let target_cycles = (a0 as u128).saturating_mul(tsc_freq as u128) / 1_000_000_000;
    let start = crate::ring0::cpu::rdtsc();
    while (crate::ring0::cpu::rdtsc() - start) < target_cycles as u64 {
        core::hint::spin_loop();
    }
    0
}
...
fn sys_ipc_send(_nr: u64, dst_pid: u64, msg_ptr: u64, msg_len: u64, _a3: u64, _a4: u64, _a5: u64) -> u64 {
    ...
    let dst = match crate::ring0::proc::process::get_process(crate::ring0::proc::process::Pid(dst_pid as u32)) {
        Some(p) => p as *mut crate::ring0::proc::process::Process,
        None => return u64::MAX,
    };
    unsafe {
        // Allocate message node on heap
        let layout = core::alloc::Layout::new::<crate::ring0::proc::process::MsgNode>();
        let node = crate::ring0::mm::slab::heap_alloc(layout.size(), layout.align())
            as *mut crate::ring0::proc::process::MsgNode;
        if node.is_null() { return u64::MAX; }

        // Allocate data buffer
        let data = crate::ring0::mm::slab::heap_alloc(msg_len as usize, 8);
        if data.is_null() {
            crate::ring0::mm::slab::heap_free(node as *mut u8, layout.size(), layout.align());
            return u64::MAX;
        }
...
fn sys_ipc_recv(_nr: u64, buf_ptr: u64, buf_len: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64) -> u64 {
    ...
    let cur = match crate::ring0::proc::task::current() {
        Some(t) => t,
        None => return u64::MAX,
    };
    let proc = match crate::ring0::proc::process::get_process(cur.pid) {
        Some(p) => p,
        None => return u64::MAX,
    };
    ...
    unsafe {
        ...
        crate::ring0::mm::slab::heap_free((*node).data, data_layout.size(), data_layout.align());
        let node_layout = core::alloc::Layout::new::<crate::ring0::proc::process::MsgNode>();
        crate::ring0::mm::slab::heap_free(node as *mut u8, node_layout.size(), node_layout.align());
...
// BMO Channel syscalls (0x34/0x35/0x36) were removed in Ultra_kernel:
// the legacy `bmo_channel` crate is no longer a dependency. The syscall
// numbers are reserved as stubs to keep userland ABI stable.
...
fn sys_beep(_nr: u64, freq: u64, duration_ms: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64) -> u64 {
    crate::ring0::dev::pc_speaker::beep(freq as u32, duration_ms as u32);
    0
}
...
fn sys_keyboard_poll(_nr: u64, _a0: u64, _a1: u64, _a2: u64, _a3: u64, _a4: u64, _a5: u64) -> u64 {
    crate::ring0::irq::keyboard::pop_scancode().unwrap_or(0) as u64
}
...
unsafe fn init_table() {
    ...
    SYSCALL_TABLE[0x34] = stub;
    SYSCALL_TABLE[0x35] = stub;
    SYSCALL_TABLE[0x36] = stub;
...
extern "C" fn syscall_handler_rust(frame: *mut InterruptFrame) {
    unsafe {
        if !RING3_ALIVE.load(Ordering::Relaxed) {
            ...
            crate::ring0::dev::console::serial_write("[syscall] first syscall received; Ring 3 is alive\n");
        }
