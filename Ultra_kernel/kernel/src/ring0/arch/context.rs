...
//! `crate::ring0::mm::phys::free_count` → `ctx.memory.free_pages`
//! `crate::ring0::cpu::tsc_per_sec`     → `ctx.cpu.tsc_freq_hz`
//! `bmo_abi clock init`          → `ctx.bmo_abi_initialized`
//! `crate::ring0::mm::heap::heap_total`      → `ctx.memory.heap_total`
//!
//! ## Stability
//!
...
/// `BootInfo` is the legacy name for the boot context. We alias the
/// local `BootContext` so the rest of `context.rs` (which uses the
/// `BootInfo` name in its public API) compiles unchanged.
pub type BootInfo = BootContext;
