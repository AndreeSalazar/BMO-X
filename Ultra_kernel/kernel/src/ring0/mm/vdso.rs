...
pub fn init() {
    ...
    unsafe {
        ...
        vdso.tsc_freq_hz = crate::ring0::cpu::tsc_per_sec();
        vdso.monotonic_tsc = AtomicU64::new(crate::ring0::cpu::rdtsc());
    ...
    crate::ring0::dev::console::serial_write("[vdso] page initialized at phys=0x");
    crate::ring0::dev::console::serial_write_u64(unsafe { VDSO_PHYS }, 16);
...
pub fn tick() {
    unsafe {
        if let Some(vdso) = VDSO_PTR.as_mut() {
            vdso.monotonic_tsc.store(crate::ring0::cpu::rdtsc(), Ordering::Relaxed);
            vdso.realtime_ns.store(crate::ring0::dev::timer::now_ns(), Ordering::Relaxed);
        }
    }
}
