...
pub(crate) fn guia(dsk: &mut Desktop, p: &bmo::Pantalla) -> After {
    ...
    s.with_ink(INK_PLAIN);
    s.text(b"    ESTRATOS no guarda archivos todavia. Sella --sube la generacion\n");
    s.text(b"    y sobrevive a reiniciar-- y se lee, pero escribir CONTENIDO es el\n");
    s.text(b"    paso que falta para su 1.0. Por eso F12 sale vacio: no es que se\n");
    s.text(b"    hayan perdido.\n");
    s.text(b"    Los archivos de ls, cat y write viven en la FAT32 (datos/, cobol/,\n");
    s.text(b"    c/, apps/), que es la particion que Windows tambien sabe leer.\n");
    s.with_ink(INK_ECHO);
